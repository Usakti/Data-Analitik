...
data_azhar=read.csv("D:/Azhar/Data Analitik/Praktikum 3/dagingsapi.csv")
df <- data_azhar %>% group_by(Provinsi) %>% summarize(median_price = median(Harga.Daging.Sapi), transactions = n(), latitude = mean(Latitude), longitude = mean(Longitude))
...
sac_borders <- c(bottom  = min(data_azhar$Latitude)  - 0.1 * height, 
                 top     = max(data_azhar$Latitude)  + 0.1 * height,
                 left    = min(data_azhar$Longitude) - 0.1 * width,
                 right   = max(data_azhar$Longitude) + 0.1 * width)
